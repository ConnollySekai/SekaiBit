...
@section('content')
    <div class="flex h-screen justify-center items-center">
        <div class="column w-full sm:w-1/2 md:w-1/3 mx-auto">
            <h1 class="text-center text-2xl">{{ trans('translations.register') }}</h1>
            <form method="POST" action="{{ route('register') }}" class="panel shadow-normal px-4 py-6">
                @csrf
                ...
                <div class="form-group">
                    <button type="submit" class="btn btn--full btn--primary">{{ trans('translations.register') }}</button>
                </div>
            </form>
        </div>
    </div>
